            ...
            for dataset_name, entries in datasets_map.items():
                for entry in entries:
                    resource_records.append({
                        ...
                        "dataset": dataset_name,
                        "seed": entry["seed"],
                        # Timing sourced from CPU time (contention-robust on the
                        # shared cluster), not wall clock. See _extract_resource_columns.
                        "train_time": sum(entry.get("training_cpu_time", {}).values()),
                        "synth_fit_time": entry.get("training_cpu_time", {}).get("synthesis_fit"),
                        "fhe_fit_time": entry.get("training_cpu_time", {}).get("training_fit"),
                        "fhe_compile_time": entry.get("training_cpu_time", {}).get("training_compile"),
                        "inf_time_total": entry.get("inference_time", {}).get("cpu_total"),
                        "inf_time_per_sample": entry.get("inference_time", {}).get("cpu_per_sample"),
